...
lm_df_lag <- final_df %>% arrange(country, year) %>% 
                          mutate(GDP_gr_rate_pre = lag(GDP_growth_rate),  # the very previous gdp growth rate
...
lm_df_lag %>% filter(country == country_pre &      # if not matched, delete
                ...
                lm(data=., log(TO_export) ~ GDP_growth_rate + GDP_gr_rate_pre + Inflation_rate + Inflation_rate_pre) %>% 
                summary()

# add auto regression variable