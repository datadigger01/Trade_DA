...
merged_data <- country_info %>%
  left_join(wdi_export1, by = c("iso_2" = "iso2c")) %>%
...
merged_data <- merged_data %>%
  left_join(wdi_import1, by = c("iso_2" = "iso2c", "year" = "year")) %>%
...
# calculate mean of gdp_growth rate by region
...
# Another way of performing independent sample t-test using pipe operator
###############################################################################
merged_data %>%
  filter(region %in% c("Asia","Americas"), year==2020) %>%
  select(name, iso_3, region, year, gdp_growth) %>% drop_na(gdp_growth) %>%
  with(t.test(gdp_growth ~ region, data = .))
...
##############################################################################################
unique(merged_data$region)
merged_data %>% filter(year %in% c(2019,2023), region %in% c('Asia')) %>% 
  ...
  # geom_vline(xintercept =3.03, color='blue') +
  theme_bw()

# # calculate mean of gdp_growth rate
merged_data %>% filter(year %in% c(2019, 2023), region %in% c("Americas"))  %>%
...
paired_t_test_data <- merged_data %>%
  filter(region %in% c("Americas"), year %in% c(2019, 2023)) %>% 
  ...
  drop_na(gdp_growth_2019, gdp_growth_2023)
# perform paired t-test
t.test(paired_t_test_data$gdp_growth_2023, paired_t_test_data$gdp_growth_2019, paired = TRUE)


# Another way of performing paired t-test using pipe operator
#################################################################
unique(merged_data$region)
merged_data %>% 
  filter(region == "Americas", year %in% c(2019, 2023)) %>%
  # filter(!iso_3 %in% c('PSE','VEN')) %>%
  select(name, iso_3, region, sub_region, year, gdp_growth) %>% drop_na(gdp_growth) %>%
  pivot_wider(id_cols = c(name,iso_3,region), names_from = year, values_from = gdp_growth, names_prefix = "gdp_growth_") %>%
  drop_na(gdp_growth_2019, gdp_growth_2023) %>%
  with(t.test(gdp_growth_2023,gdp_growth_2019, paired = TRUE, data = .))
